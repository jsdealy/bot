...
async def grab(interaction: discord.Interaction,genre: str,language: str,visibility: int):
    try:
        ...
        cannes_films = []
        gen_films = []
        # lang and genre selected <== 12/13/24 19:29:04 # 
        ...
        # lang selected, genre any <== 12/13/24 19:29:16 # 
        ...
        # any lang, any genre <== 12/13/24 19:29:25 # 
        elif language.startswith("any") and genre.lower().startswith("any"):
            cannes_films = select(con.cur(),"Films.title","Films.tconst",tables=["Cannes","Films","Ratings"],joins=["Ratings.tconst=Cannes.tconst","Films.tconst=Cannes.tconst"],qualifiers=[f"AND Ratings.rating > 6.5 AND Ratings.numVotes > 3000 ORDER BY RANDOM() LIMIT {CANNES_LIMIT}"])
            criterion_films = select(con.cur(),"Films.title","Films.tconst",tables=["Criterion","Films","Ratings"],joins=["Ratings.tconst=Criterion.tconst","Films.tconst=Criterion.tconst"],qualifiers=[f"AND Ratings.rating > 6.5 AND Ratings.numVotes > 3000 ORDER BY RANDOM() LIMIT {CRITERION_LIMIT}"])
        ...
        # any lang, genre selected <== 12/13/24 19:29:47 # 
        else:
